...
Mean_AV <- parcels %>%
  mutate(AVPerAcre = Net_AV/Acreage) %>% 
  select(AVPerAcre) %>% 
  filter(AVPerAcre != "Inf") %>% 
  filter(AVPerAcre != "NaN")

  #mutate(Net_AV = as.numeric(Net_AV)) %>% 
  #mutate(Acreage = as.numeric(Acreage)) %>% 
#mutate(AVPerAcre = as.numeric(AVPerAcre))
