...
fish_summary = function(catch_sides_data, price_data, graph=FALSE) {
  ...
  revenues_sideofisland <- left_join(catch_sides_data, price_data, by = "fish") %>%
    mutate(west_rev = caughtwest*price) %>% 
    mutate(north_rev = caughtnorth*price) %>%
    mutate(east_rev = caughteast*price)
  ...
  fish_west <- rep(catch_sides_data$fish, catch_sides_data$caughtwest)
  fish_north <- rep(catch_sides_data$fish, catch_sides_data$caughtnorth)
  fish_east <- rep(catch_sides_data$fish, catch_sides_data$caughteast)
